...
export const ViewAddProductsQuoter = () => {
  const { products, activeQuoter, priceDiscountQuoter, isAddProductQuoterProcess, quoterProcess } = useSelector(
    (state) => state.quoter
  );
  ...
  const onSearchProduct = ({ target }) => {
    ...
    productToSearch != '' ? setIsSearching(true) : setIsSearching(false);

    const productsMatches = products.filter((element) => {
      if (element.title.toLowerCase().includes(productToSearch)) return true;
    });
  ...
  const buttonApplyChanges = (
    <Button variant="outlined" fullWidth onClick={updateQuoter} color="primary" sx={{ padding: 1 }}>
  ...
  return (
    <Container maxWidth="sm">
      ...
      <List>
        {isSearching
          ? productsMatches2.map((product) => <AddProductQuoterItem key={product.id} {...product} />)
          : products.map((product) => <AddProductQuoterItem key={product.id} {...product} />)}
      </List>
      {buttonApplyChanges}
    </Container>
  );

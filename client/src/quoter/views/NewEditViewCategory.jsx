...
export const NewEditViewCategory = () => {
  const { errorMessage, successMessage, activeCategory, statusQuoter, quoterProcess } = useSelector(
    (state) => state.quoter
  );
  ...
  const onClickSaveCategory = (event) => {
    ...
    quoterProcess === 'edit' ? dispatch(startUpdateCategory(formState)) : dispatch(startCreateCategory(formState));
  };
  ...
  useEffect(() => {
    if (successMessage) Swal.fire({ icon: 'success', title: successMessage, showConfirmButton: false, timer: 1500 });
  }),
    [successMessage];
  ...
  const onDeleteCategory = (event) => {
    event.preventDefault();
    Swal.fire(configConfirmSwal).then((result) => {
  ...
  return (
    <>
      <form onSubmit={onClickSaveCategory}>
        <Box sx={{ flexGrow: 1 }}>
          <Grid
            container
            direction="row"
          ...
          >
            <Grid item>
              <Typography fontSize={39} fontWeight="light">
                {' '}
                {user.rol == 'user' ? 'View' : quoterProcess} Category
              </Typography>
            </Grid>
            <Grid item>
              <Button
              ...
              <Button
                ...
                disabled={statusQuoter == 'communicating' || quoterProcess !== 'Edit'}

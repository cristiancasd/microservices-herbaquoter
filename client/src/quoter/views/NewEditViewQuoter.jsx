...
export const NewEditViewQuoter = () => {
  ...
  const { title, description, formState, isFormValid, titleValid, onInputChange, onResetForm } = useForm(
    activeQuoterToEdit,
    formValidations
  );
  ...
  const [pricesQuoter, setPricesQuoter] = useState([
    'pricepublic',
    'price15',
    'price25',
    'price35',
  ...
  useEffect(() => {
    if (errorMessage !== undefined && errorMessage !== null) Swal.fire('Error', errorMessage, 'error');
  }, [errorMessage]);
        ...
        <Grid container spacing={2} alignItems="center">
          ...
          <Grid item xs={12} md={9}>
            <TextField
              ...
              variant="filled"
              inputProps={
                quoterProcess === 'View' && !activeQuoter.isDefaultQuoter ? { readOnly: true } : { readOnly: false }
              }
              fullWidth
              name="title"
              value={title}

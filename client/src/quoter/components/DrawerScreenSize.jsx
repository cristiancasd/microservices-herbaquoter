...
export const DrawerScreenSize = (data) => {
  ...
  const { mobileOpen, isScreenCel, categories, products, quoters, activeQuoter, navBarSelection, quotersDefault } =
  ...
  const onSearch = ({ target }) => {
    ...
    if (navBarSelection === 'quoters') {
      const quotersMatchesUser = quoters.filter((element) => {
        if (element.title.toLowerCase().includes(toSearch)) return true;
      });
      quotersMatchesUser.map((quoter) => toShowMenuSearch.push(<SideBarItemQuoters key={quoter.id} {...quoter} />));
      ...
      productsMatches.map((product) => toShowMenuSearch.push(<SideBarItemProducts key={product.id} {...product} />));
    }
  ...
  useEffect(() => {
    ...
    quoters.map((quoter) => menuQuotersTemporal.push(<SideBarItemQuoters key={quoter.id} {...quoter} />));
